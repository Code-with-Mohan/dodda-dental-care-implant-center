(function($) {
    ...
    if ($('.main-navigation .navigation-box .submenu').length) {
        ...
        subMenu.parent('li').children('a').append(function() {
            return '<button class="sub-nav-toggler"> <span class="sr-only">Toggle navigation</span> <span class="fa fa-angle-double-down"></span> </button>';
        });
    ...
    // Custom WhatsApp redirection for forms
    $('form.appointment-one__form, form._c_form_sub').on('submit', function(e) {
        var $form = $(this);
        var name = $form.find('[name="name"]').val() || '';
        var phone = $form.find('[name="phone"], [name="mobile"]').val() || '';
        var email = $form.find('[name="email"]').val() || '';
        var date = $form.find('[name="dt-local"]').val() || '';
        var message = $form.find('[name="message"], [name="msg"]').val() || '';
        
        var whatsappMessage = "New Inquiry from Website:\n" +
            "Name: " + name + "\n" +
            "Phone: " + phone + "\n" +
            "Email: " + email + "\n";
            
        if (date) {
            whatsappMessage += "Preferred Appointment: " + date + "\n";
        }
        
        whatsappMessage += "Message: " + message;
        
        var whatsappUrl = "https://web.whatsapp.com/send?phone=919494459442&text=" + encodeURIComponent(whatsappMessage);
        window.open(whatsappUrl, '_blank');
        
        // Return true to allow the form to also submit normally if required, 
        // or false to only send via WhatsApp. 
        // Given the request "redirect to the whatsapp", returning true might interfere with the redirect.
        // However, window.open with _blank will open a new tab.
        // If the user wants a true redirect in the same tab, I'd use location.href.
        // I'll use window.open for now as it's safer for users.
        return true; 
    });
